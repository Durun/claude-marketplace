...
1 行目はオヤジ。問題にした語を必ず含めて、上の例の口調で叫ぶ。
2 行目はババア。隣にいるババアがオヤジをたしなめつつ、問題にした語を回答の文脈から読み解いて言い直す。
「まあ落ち着いて。」で始め、続けて **「面」** は **「画面の描画領域」** のことだと思うわよ。のように、語ごとに言い換えを添える。
指摘が語ではなく文の書き方のときは、書き手が言いたかったことを補って言う。
穏やかな口調で 1 〜 2 文。叫ばない。話者名は書かない。

2 行の出力の例:
**「面」**って一体何のことダァ**〜〜！？**
まあ落ち着いて。**「面」**は**「画面の描画領域」**のことだと思うわよ。
...
export const register: Register = (on) => {
  ...
  on('turn.complete', async ($, e, next) => {
    ...
    // 一言目で区切って、解説を次の行に落とす。
    tashiname = tsukkomi ? (lines.slice(1).join('').replace(/^(まあ落ち着いて。)/, '$1\n') || null) : null
